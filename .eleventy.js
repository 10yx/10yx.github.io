...
module.exports = async function (eleventyConfig) {
    ...
    const { default: syntaxHighlight } = await import('@11ty/eleventy-plugin-syntaxhighlight');
    const { default: markdownIt } = await import('markdown-it');
    const { default: markdownItEmoji } = await import('markdown-it-emoji');
    const { default: pluginSvgSprite } = await import("eleventy-plugin-svg-sprite");

    const { default: filters } = await import('./utils/filters.js');
    const { default: shortcodes } = await import('./utils/shortcodes.js');
    const { default: pairedshortcodes } = await import('./utils/paired-shortcodes.js');
    ...
    */
    ...
    eleventyConfig.addCollection('post', (collectionApi) => {
        if (process.env.ELEVENTY_ENV !== 'production')
            ...
            return collectionApi.getFilteredByGlob('./src/posts/*.md').filter((post) => !post.data.draft);
    })
    ...
    eleventyConfig.addCollection('tagList', (collectionApi) => {
        const includedTags = new Set(['authors', 'pages', 'landing', 'post']);

        const tagSet = new Set(
            collectionApi.getAll()
                .flatMap(item => (item.data.tags || []))
                .filter(tag => includedTags.has(tag))
        );
